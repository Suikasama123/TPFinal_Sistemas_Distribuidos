...
// Configuración
const MQTT_BROKER = process.env.MQTT_BROKER_URL || process.env.MQTT_BROKER || 'mqtt://mosquitto:1883';
...
function loadApiKeys() {
  try {
    const configPath = '/app/config/api_keys.json';
...
function getNextApiKey() {
  ...
  console.log(`[CONFIG] 🔑 Usando API key: ${selectedKey.id} (${selectedKey.owner}) - Provider: ${selectedKey.provider}`);
  return {
    key: selectedKey.key,
    provider: selectedKey.provider || 'gemini'
  };
}
...
function assignTaskToWorker(workerId, task) {
  ...
  const apiKeyData = task.apiKey ? { key: task.apiKey, provider: 'gemini' } : getNextApiKey();
  ...
  console.log(`[TASK] Asignando tarea a worker ${workerId} con provider ${apiKeyData.provider}`);
  publishLog(`Master asignó tarea a worker ${workerId} para sesión ${task.sessionId} (${apiKeyData.provider})`);
