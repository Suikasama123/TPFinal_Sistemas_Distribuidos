...
def query_ai(query, api_key, provider="gemini"):
    """Realizar consulta a AI (soporta Gemini y DeepSeek)"""
    try:
        if provider == "deepseek":
            return query_deepseek(query, api_key)
        else:  # gemini por defecto
            return query_gemini(query, api_key)
    except Exception as e:
        error_msg = f"Error al consultar {provider}: {str(e)}"
...
def query_deepseek(query, api_key):
    """Realizar consulta a DeepSeek AI"""
    publish_log(f"Worker {WORKER_ID} consultando a DeepSeek AI")
...
def process_task(task_data):
    ...
    try:
        # Detectar provider de la key
        provider = task_data.get('provider', 'gemini')
        
        # 1. Consultar a AI
        ai_response = query_ai(task_data['query'], task_data['api_key'], provider)
